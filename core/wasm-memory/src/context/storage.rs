...
pub(crate) fn init_rocksdb() -> anyhow::Result<DB> {
    // TODO
    let path = "/tmp";
    let secret_cf = ColumnFamilyDescriptor::new("secret_key", Options::default());
    let public_cf = ColumnFamilyDescriptor::new("public_key", Options::default());
    let mut db_opts = Options::default();
    db_opts.create_missing_column_families(true);
    db_opts.create_if_missing(true);
    DB::open_cf_descriptors(&db_opts, path, vec![secret_cf, public_cf])
...
//     let value = context
//         .db
//         .get_cf(context.db.cf_handle("secret_key").unwrap(), key)
...
//     context
//         .db
//         .delete_cf(context.db.cf_handle("public_key").unwrap(), key)
//         .map_err(|e| StorageError(e.to_string()))?;
//     Ok(())
// }
